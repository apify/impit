...
impl<CookieStoreImpl: CookieStore + 'static> Impit<CookieStoreImpl> {
    ...
    fn new(config: ImpitBuilder<CookieStoreImpl>) -> Result<Self, ImpitError> {
        ...
        // Set pseudo-header order from fingerprint via env var.
        // The h2 fork reads this env var once per connection (in Builder::new())
        // and stores it per-connection, so subsequent env var changes don't affect
        // existing connections. This is a known limitation: if multiple Impit
        // instances with different browsers are created concurrently, there's a
        // small window where the env var could contain the wrong order before
        // the h2 connection is established. In practice this is very unlikely
        // with Node.js's single-threaded event loop.
        ...
        if !pseudo_headers_order.is_empty() {
            // SAFETY: This is process-global but read once per h2 connection.
            // In the NAPI context, JS is single-threaded, so this is set
            // synchronously before the async request triggers h2 connection setup.
            unsafe {
                std::env::set_var(
        ...
        // Set MAX_CONCURRENT_STREAMS via env var for h2 fork.
        // Chrome sends MAX_CONCURRENT_STREAMS=1000, Firefox doesn't send it.
        // reqwest doesn't expose this setting, so we pass it via env var.
        if let Some(ref fingerprint) = config.fingerprint {
            if let Some(max_concurrent) = fingerprint.http2.settings_max_concurrent_streams {
                unsafe {
                    std::env::set_var(
                        "IMPIT_H2_MAX_CONCURRENT_STREAMS",
                        max_concurrent.to_string(),
                    );
                }
            } else {
                // If the fingerprint doesn't specify it (Firefox), remove the env var
                // so h2 won't include it in SETTINGS.
                unsafe {
                    std::env::remove_var("IMPIT_H2_MAX_CONCURRENT_STREAMS");
                }
            }
        }
